...
class PipelineConfig:
    ...
    Only LIB_R1 / LIB_R2 are required (forward and reverse share the same
    sequencing data). Software paths are auto-detected from $PATH unless
    explicitly overridden in config.
    """
    prefix: str
    outdir: Path

    # Sequencing data (shared by both orientations)
    lib_r1: Path
    lib_r2: Path

    # Orientation tags (reverse complement as required by remove_ODN.pl logic)
    forward_tag: str
    reverse_tag: str
    ...
    def samples(self) -> list[tuple[str, str]]:
        """Return (sample_name, tag) for both orientations."""
        return [
            (self.sample_name("plus"), self.forward_tag),
...
class SampleReport:
    ...
    dedup_unique: int = 0
    usable_reads: int = 0
    forward_targets: int = 0
    reverse_targets: int = 0
